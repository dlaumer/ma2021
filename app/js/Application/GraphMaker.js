...
/*
--------------
Application/GraphMaker.js
--------------
Holds all the data for making the bar and pie charts using D3.js. There is a function to initalize the 
graphs and then two different functions to update them, one for public transport and one for traffic.
...
define([
    ...
    return Accessor.createSubclass({
        ...
        /**** INITIALIZING *************************************/
        // Initialize the bar graph, make all axis and define size
        ...
        // Initialize the second diagram (either donut or circle diagram)
        ...
        // Update bar chart for public transport info
        updatePTDiagramm: function (hourData, firstTime = false) {
            ...
            // Add another set of bars on top to fill up to 100%
        ...
        // Update bar chart for traffic info
        updateTrafficDiagramm: function (hourData, firstTime = false) {
            ...
            // Add Y axis
            that.yScale.domain([0, 1800]); // 1800 is a bit more than the maximum value of the traffic data
            that.yAxis.conditionalTransition(firstTime)
                .attr("class", "axis")
                .call(d3.axisLeft(that.yScale).ticks(3));
        ...
        // Remove all diagramns (when the theme is de-selected)
        removeDiagrams: function () {
            if (dom.byId("svgTimeline")) {
                dom.byId("svgTimeline").remove(); // To remove the old one.
            }
            if (dom.byId("svgDonut")) {
                dom.byId("svgDonut").remove(); // To remove the old one.
            }
